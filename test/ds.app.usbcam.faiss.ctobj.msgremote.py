...
def nvsink_sink_pad_buffer_probe(osd_sink_pad, info, u_data):
    """
    nvosd sink 的 buffer probe: 为人脸挂载 CTFaceObjectMeta 到 frame_user_meta_list.
    buffer 经 tee 分叉后, 一路 queue1->msgconv->msgbroker; 标准 nvmsgconv 只消费 NvDsEventMsgMeta,
    不消费本处挂的 CTFaceObjectMeta, 需配合 NvDsEventMsgMeta 或自定义 payload 库才能发到 Redis.
    """
    ...
    try:
        ...
        while l_frame is not None:
            ...
            # 有人脸时挂一个 NvDsEventMsgMeta，在 extMsg 中存 batch_meta 指针，供自定义 msg2p 从
            # frame_user_meta_list 取 CTFaceObjectMeta 并生成 payload，否则 nvmsgconv 不会调用自定义库
            if face_obj_metas:
                ...
                if user_event_meta:
                    ...
                    pyds.nvds_event_msg_meta_set_batch_pointer(msg_meta, batch_meta)
    ...
    try:
        ...
        msgconv = acquire_nvmsgconv(
            ...
            args={
                ...
                "payload_type": 257,
                "msg2p_lib": "/home/good/wkspace/deepstream-sdk/deepstream_python_apps/apps/dsapp/libs/nvds_msg2p_ctface.so",
                # 无 NvDsEventMsgMeta 时也尝试生成 payload，才会调用自定义 msg2p
                "dummy_payload": True,
                # 勿开 msg2p_newapi：插件会传 NvDsEvent* 而非 NvDsBatchMeta*, 本库按 batch 解析会段错误
            },
        )
